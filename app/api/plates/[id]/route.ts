...
export async function GET(request: NextRequest, { params }: RouteContext) {
  try {
    ...
    const response = await fetch(`${BACKEND_API_URL}/plates/${id}`, {
      method: 'GET',
      headers: {
    ...
    const { data, ok } = await fetchJson(response, 'Plate GET by id')

    if (!ok) {
      return NextResponse.json(
        { success: false, error: errorMessage(data, 'Failed to fetch plate') },
    ...
    console.error('Plate GET by id API error:', error)
...
export async function PATCH(request: NextRequest, { params }: RouteContext) {
  try {
    ...
    const { data, ok } = await fetchJson(response, 'Plate PATCH')

    if (!ok) {
      return NextResponse.json(
        { success: false, error: errorMessage(data, 'Failed to update plate') },
        { status: response.status }
      )
    }
    ...
    console.error('Plate PATCH API error:', error)
    return NextResponse.json({ success: false, error: 'Internal server error' }, { status: 500 })
  }
}
...
export async function DELETE(request: NextRequest, { params }: RouteContext) {
  try {
    ...
    if (!response.ok) {
      ...
      return NextResponse.json(
        { success: false, error: errorMessage(data, 'Failed to delete plate') },
        { status: response.status }
      )
    }
    ...
    console.error('Plate DELETE API error:', error)
    return NextResponse.json({ success: false, error: 'Internal server error' }, { status: 500 })
  }
}
