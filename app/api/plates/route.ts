...
export async function GET(request: NextRequest) {
  try {
    ...
    if (!BACKEND_API_URL) {
      console.error('Plates GET - BACKEND_API_URL is not configured')
    ...
    const url = `${BACKEND_API_URL}/plates/${qs ? `?${qs}` : ''}`
    ...
    const { data, ok } = await fetchJson(response, 'Plates GET')

    if (!ok) {
      return NextResponse.json(
        { success: false, error: errorMessage(data, 'Failed to fetch plates') },
    ...
    console.error('Plates GET API error:', error)
...
export async function POST(request: NextRequest) {
  try {
    const BACKEND_API_URL = process.env.BACKEND_API_URL
    if (!BACKEND_API_URL) {
      console.error('Plates POST - BACKEND_API_URL is not configured')
      return NextResponse.json({ success: false, error: 'BACKEND_API_URL is not configured' }, { status: 500 })
    }
    ...
    const response = await fetch(`${BACKEND_API_URL}/plates/`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
        Authorization: `Bearer ${session.sessionToken}`,
    ...
    const { data, ok } = await fetchJson(response, 'Plates POST')

    if (!ok) {
      return NextResponse.json(
        { success: false, error: errorMessage(data, 'Failed to create plate') },
        { status: response.status }
      )
    }
    ...
    console.error('Plates POST API error:', error)
    return NextResponse.json({ success: false, error: 'Internal server error' }, { status: 500 })
  }
}
