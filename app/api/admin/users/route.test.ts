jest.mock('next/server', () => {
  ...
  return { NextRequest: function () {}, NextResponse }
})
...
describe('GET /api/admin/users', () => {
  beforeEach(() => {
    jest.clearAllMocks()
    ;(getAdminBackendUrl as jest.Mock).mockReturnValue('https://api.example.com/admin/users')
  ...
  it('returns admin error when session is not admin', async () => {
    ;(getAdminSession as jest.Mock).mockResolvedValue(
      NextResponse.json({ error: 'Forbidden' }, { status: 403 })
    )
    const request = { nextUrl: { searchParams: { get: () => null } } } as unknown as NextRequest
    const response = await GET(request)
    expect(response.status).toBe(403)
  })

  it('returns users data when backend succeeds', async () => {
    ...
    const request = { nextUrl: { searchParams: { get: () => '10' } } } as unknown as NextRequest
    const response = await GET(request)
