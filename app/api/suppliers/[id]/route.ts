...
export async function GET(request: NextRequest, { params }: RouteContext) {
  try {
    ...
    const response = await fetch(`${BACKEND_API_URL}/suppliers/${id}`, {
      method: 'GET',
      headers: {
    ...
    const { data, ok } = await fetchJson(response, 'Supplier GET by id')

    if (!ok) {
      return NextResponse.json(
        { success: false, error: errorMessage(data, 'Failed to fetch supplier') },
    ...
    console.error('Supplier GET by id API error:', error)
...
export async function PATCH(request: NextRequest, { params }: RouteContext) {
  try {
    ...
    const { data, ok } = await fetchJson(response, 'Supplier PATCH')

    if (!ok) {
      return NextResponse.json(
        { success: false, error: errorMessage(data, 'Failed to update supplier') },
        { status: response.status }
      )
    }
...
export async function DELETE(request: NextRequest, { params }: RouteContext) {
  try {
    ...
    if (!response.ok) {
      ...
      return NextResponse.json(
        { success: false, error: errorMessage(data, 'Failed to delete supplier') },
        { status: response.status }
      )
    }
