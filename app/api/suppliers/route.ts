...
export async function GET(request: NextRequest) {
  try {
    ...
    if (!BACKEND_API_URL) {
      console.error('Suppliers GET - BACKEND_API_URL is not configured')
    ...
    const url = `${BACKEND_API_URL}/suppliers/${qs ? `?${qs}` : ''}`
    ...
    const { data, ok } = await fetchJson(response, 'Suppliers GET')

    if (!ok) {
      return NextResponse.json(
        { success: false, error: errorMessage(data, 'Failed to fetch suppliers') },
...
export async function POST(request: NextRequest) {
  try {
    const BACKEND_API_URL = process.env.BACKEND_API_URL
    if (!BACKEND_API_URL) {
      console.error('Suppliers POST - BACKEND_API_URL is not configured')
      return NextResponse.json({ success: false, error: 'BACKEND_API_URL is not configured' }, { status: 500 })
    }
    ...
    const response = await fetch(`${BACKEND_API_URL}/suppliers/`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
        Authorization: `Bearer ${session.sessionToken}`,
    ...
    const { data, ok } = await fetchJson(response, 'Suppliers POST')

    if (!ok) {
      return NextResponse.json(
        { success: false, error: errorMessage(data, 'Failed to create supplier') },
        { status: response.status }
      )
    }
