...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!BACKEND_API_URL) {
      console.error('Ingredient AI suggestion POST - BACKEND_API_URL is not configured')
    ...
    const response = await fetch(`${BACKEND_API_URL}/ingredients/ai/market-suggestion`, {
      method: 'POST',
      headers: {
    ...
    const { data, ok } = await fetchJson(response, 'Ingredient AI suggestion POST')

    if (!ok) {
      return NextResponse.json(
        { success: false, error: errorMessage(data, 'Failed to get AI suggestion') },
    ...
    console.error('Ingredient AI suggestion POST API error:', error)
