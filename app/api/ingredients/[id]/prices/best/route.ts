...
export async function GET(request: NextRequest, { params }: RouteContext) {
  try {
    ...
    if (!BACKEND_API_URL) {
      console.error('Ingredient prices/best GET - BACKEND_API_URL is not configured')
    ...
    const response = await fetch(`${BACKEND_API_URL}/ingredients/${id}/prices/best`, {
      headers: { Authorization: `Bearer ${session.sessionToken}` },
      cache: 'no-store',
    })

    const { data, ok } = await fetchJson(response, 'Ingredient prices/best GET')
    if (!ok) {
      return NextResponse.json(
        { success: false, error: errorMessage(data, 'Failed to load best prices') },
    ...
    console.error('Ingredient prices/best GET API error:', error)
