...
export async function GET(request: NextRequest, { params }: RouteContext) {
  try {
    ...
    const { data, ok } = await fetchJson(response, 'Ingredient GET by id')

    if (!ok) {
      return NextResponse.json(
        { success: false, error: errorMessage(data, 'Failed to fetch ingredient') },
    ...
    console.error('Ingredient GET by id API error:', error)
...
export async function PATCH(request: NextRequest, { params }: RouteContext) {
  try {
    ...
    const { data, ok } = await fetchJson(response, 'Ingredient PATCH')

    if (!ok) {
      return NextResponse.json(
        { success: false, error: errorMessage(data, 'Failed to update ingredient') },
        { status: response.status }
      )
    }
    ...
    console.error('Ingredient PATCH API error:', error)
    return NextResponse.json({ success: false, error: 'Internal server error' }, { status: 500 })
  }
}
...
export async function DELETE(request: NextRequest, { params }: RouteContext) {
  try {
    ...
    if (!response.ok) {
      ...
      return NextResponse.json(
        { success: false, error: errorMessage(data, 'Failed to delete ingredient') },
        { status: response.status }
      )
    }
    ...
    console.error('Ingredient DELETE API error:', error)
    return NextResponse.json({ success: false, error: 'Internal server error' }, { status: 500 })
  }
}
