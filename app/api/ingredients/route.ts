...
export async function GET(request: NextRequest) {
  try {
    ...
    if (!BACKEND_API_URL) {
      console.error('Ingredients GET - BACKEND_API_URL is not configured')
    ...
    const url = `${BACKEND_API_URL}/ingredients/${qs ? `?${qs}` : ''}`
    ...
    const { data, ok } = await fetchJson(response, 'Ingredients GET')

    if (!ok) {
      return NextResponse.json(
        { success: false, error: errorMessage(data, 'Failed to fetch ingredients') },
    ...
    console.error('Ingredients GET API error:', error)
...
export async function POST(request: NextRequest) {
  try {
    const BACKEND_API_URL = process.env.BACKEND_API_URL
    if (!BACKEND_API_URL) {
      console.error('Ingredients POST - BACKEND_API_URL is not configured')
      return NextResponse.json({ success: false, error: 'BACKEND_API_URL is not configured' }, { status: 500 })
    }
    ...
    const { data, ok } = await fetchJson(response, 'Ingredients POST')

    if (!ok) {
      return NextResponse.json(
        { success: false, error: errorMessage(data, 'Failed to create ingredient') },
        { status: response.status }
      )
    }
    ...
    console.error('Ingredients POST API error:', error)
    return NextResponse.json({ success: false, error: 'Internal server error' }, { status: 500 })
  }
}
