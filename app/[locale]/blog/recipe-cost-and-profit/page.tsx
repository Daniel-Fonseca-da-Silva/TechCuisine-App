...
import { RecipeCostProfitContent } from '@/components/features/blog/recipe-cost-profit-content';
import { buildAlternates, buildOgImage } from '@/lib/seo';

export async function generateMetadata(): Promise<Metadata> {
  const t = await getTranslations('BlogRecipeCostPage.metadata');
  const locale = await getLocale();
  const { canonical, languages } = buildAlternates('/blog/recipe-cost-and-profit', locale);
...
export default async function BlogRecipeCostPage() {
  const allMessages = await getMessages();
  const messages: AbstractIntlMessages = {
    logo: allMessages.logo,
    cookie: allMessages.cookie,
    HomePage: allMessages.HomePage,
    BlogRecipeCostPage: allMessages.BlogRecipeCostPage,
  };
